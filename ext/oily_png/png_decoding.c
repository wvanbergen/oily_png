...
BYTE oily_png_extract_1bit_element(BYTE* bytes, int start, int x) {
...
BYTE oily_png_extract_2bit_element(BYTE* bytes, int start, int x) {
  BYTE byte = bytes[start + 1 + (x >> 2)];
  int bitshift = (6 - ((x & (BYTE) 0x03) << 1));
  return (byte & (0x03 << bitshift)) >> bitshift;
}

BYTE oily_png_extract_4bit_element(BYTE* bytes, int start, int x) {
  return ((x & 0x01) == 0) ? ((bytes[(start) + 1 + ((x) >> 1)] & (BYTE) 0xf0) >> 4) : (bytes[(start) + 1 + ((x) >> 1)] & (BYTE) 0x0f);
}

BYTE oily_png_resample_1bit_element(BYTE* bytes, int start, int x) {
  BYTE value = oily_png_extract_1bit_element(bytes, start, x);
  return (value == 0) ? 0x00 : 0xff;
}

BYTE oily_png_resample_2bit_element(BYTE* bytes, int start, int x) {
...
BYTE oily_png_resample_4bit_element(BYTE* bytes, int start, int x) {
...
PIXEL oily_png_decode_pixel_indexed_8bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
  return (PIXEL) NUM2UINT(rb_funcall(decoding_palette, rb_intern("[]"), 1, INT2FIX(bytes[start + 1 + x])));
}

PIXEL oily_png_decode_pixel_indexed_4bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
  return (PIXEL) NUM2UINT(rb_funcall(decoding_palette, rb_intern("[]"), 1, INT2FIX(oily_png_extract_4bit_element(bytes, start, x))));
}

PIXEL oily_png_decode_pixel_indexed_2bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
  return (PIXEL) NUM2UINT(rb_funcall(decoding_palette, rb_intern("[]"), 1, INT2FIX(oily_png_extract_2bit_element(bytes, start, x))));
}

PIXEL oily_png_decode_pixel_indexed_1bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
  return (PIXEL) NUM2UINT(rb_funcall(decoding_palette, rb_intern("[]"), 1, INT2FIX(oily_png_extract_1bit_element(bytes, start, x))));
}


PIXEL oily_png_decode_pixel_grayscale_8bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_grayscale_1bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_grayscale_2bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_grayscale_4bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_truecolor_8bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_grayscale_alpha_8bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_truecolor_alpha_8bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_grayscale_16bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_truecolor_16bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
PIXEL oily_png_decode_pixel_grayscale_alpha_16bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
  return BUILD_PIXEL( bytes[start + 1 + (x * 4) + 0], 
                      bytes[start + 1 + (x * 4) + 0], 
                      bytes[start + 1 + (x * 4) + 0], 
                      bytes[start + 1 + (x * 4) + 2]);
...
PIXEL oily_png_decode_pixel_truecolor_alpha_16bit(BYTE* bytes, int start, int x, VALUE decoding_palette) {
...
VALUE oily_png_decode_png_image_pass(VALUE self, VALUE stream, VALUE width, VALUE height, VALUE color_mode, VALUE depth, VALUE start_pos) {
  ...
  if ((FIX2INT(height) > 0) && (FIX2INT(width) > 0)) {

    int pixel_size = oily_png_pixel_bytesize(FIX2INT(color_mode), FIX2INT(depth));
    int line_size  = oily_png_scanline_bytesize(FIX2INT(color_mode), FIX2INT(depth), FIX2INT(width));
    int pass_size  = oily_png_pass_bytesize(FIX2INT(color_mode), FIX2INT(depth), FIX2INT(width), FIX2INT(height));
    ...
    PIXEL (*pixel_decoder)(BYTE*, int, int, VALUE) = NULL;
    pixel_decoder = oily_png_decode_pixel_func(FIX2INT(color_mode), FIX2INT(depth));
  
    if (pixel_decoder == NULL) {
      rb_raise(rb_eRuntimeError, "No decoder for color mode %ld and bit depth %ld", FIX2INT(color_mode), FIX2INT(depth));
    }
    ...
    for (y = 0; y < FIX2INT(height); y++) {
      ...
      for (x = 0; x < FIX2INT(width); x++) {
        ...
        rb_ary_store(pixels, FIX2INT(width) * y + x, INT2NUM(pixel));
      }
    }
  }
