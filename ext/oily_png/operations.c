...
VALUE oily_png_compose(int argc, VALUE *argv, VALUE self) {
  // Make a duplicate of the object so that we aren't overwriting the original.
  VALUE dup = rb_funcall( self, rb_intern( "dup" ), 0 );
  return oily_png_compose_bang( argc, argv, dup );
}
